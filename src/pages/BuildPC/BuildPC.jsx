...
import { WrapperCountOrder, WrapperHeader, WrapperHeader2, WrapperItemOrder, WrapperLeft, WrapperListOrder, WrapperRight, WrapperStyleHeader, WrapperTypeProduct } from "./style";
...
const BuildPC = () => {
    ...
    const handleOnchangeCheckAll = (e) => {
        ...
        if (e.target.checked) {
            ...
            buildpc?.buildpcItems?.forEach((item) => {
    ...
    useEffect(() => {
        dispatch(selectedbuildpc({ listChecked }))
    }, [listChecked])

    const handleRemoveAllOrder = () => {
        if (listChecked?.length > 1) {
            dispatch(removeAllbuildpcProduct({ listChecked }))
        }
    }
                    ...
                    <WrapperRight>
                        <WrapperStyleHeader>
                            <span style={{ display: 'inline-block', width: '390px' }}>
                                <Checkbox onChange={handleOnchangeCheckAll} checked={listChecked?.length === buildpc?.buildpcItems?.length}></Checkbox>
                                <span> Tất cả ({buildpc?.buildpcItems?.length} sản phẩm)</span>
                            </span>
                        ...
                        <WrapperListOrder>
                            {buildpc?.buildpcItems?.map((buildpc) => {
                                console.log('build', buildpc)
                                return (
                                    <WrapperItemOrder key={buildpc?.product}>
                                        <div style={{ width: '150px', display: 'flex', alignItems: 'center', gap: 2 }}>
